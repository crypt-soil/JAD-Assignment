...
//		doGet(request, response);
        ...
        if (full == null || full.trim().isEmpty()) full = old.getFullName();
        if (phone == null || phone.trim().isEmpty()) phone = old.getPhone();
        if (addr == null || addr.trim().isEmpty()) addr = old.getAddress();
        if (zip == null || zip.trim().isEmpty()) zip = old.getZipcode();
