...
public class ConfirmCheckoutServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (!keys.next()) throw new SQLException("Failed to create booking (no generated key).");
            ...
            for (CartItem item : items) {
                ...
                if (item.getCaregiverId() == null) psDetails.setNull(3, Types.INTEGER);
                else psDetails.setInt(3, item.getCaregiverId());
         ...
         // Redirect back to home (Categories page)
            ...
            try { if (psDetails != null) psDetails.close(); } catch (Exception ignore) {}
            try { if (psBooking != null) psBooking.close(); } catch (Exception ignore) {}
