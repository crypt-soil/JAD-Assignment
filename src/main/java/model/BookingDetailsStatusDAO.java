...
public class BookingDetailsStatusDAO {
    ...
    public boolean checkIn(int detailId, int caregiverId) {
        String sql =
            ...
            "SET caregiver_status = 2, " +                       // checked_in
            "    check_in_at = COALESCE(check_in_at, NOW()), " +
            "    check_out_at = NULL " +
            "WHERE detail_id = ? " +
            "  AND caregiver_id = ? " +
            "  AND DATE(start_time) = CURDATE() " +
            "  AND caregiver_status = 1";                        // assigned
    ...
    public boolean checkOut(int detailId, int caregiverId) {
        String sql =
            "UPDATE booking_details " +
            "SET caregiver_status = 3, " +                       // checked_out
            "    check_out_at = COALESCE(check_out_at, NOW()) " +
            "WHERE detail_id = ? " +
            "  AND caregiver_id = ? " +
            "  AND DATE(start_time) = CURDATE() " +
            "  AND caregiver_status = 2";                        // checked_in
    ...
    public String[] getBookingInfoByDetailId(int detailId) {
        String sql =
            "SELECT bd.booking_id, s.name AS service_name " +
            "FROM booking_details bd " +
