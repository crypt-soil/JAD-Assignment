...
public class CartDAO {
	...
	public List<CartItem> getCartItemsByCustomerId(int customerId) {
		...
		String sql = "SELECT ci.item_id, ci.service_id, s.name, s.price, ci.quantity, "
				+ "       ci.start_time, ci.end_time, ci.caregiver_id, ci.special_request " + "FROM cart c "
				+ "JOIN cart_items ci ON c.cart_id = ci.cart_id " + "JOIN service s ON ci.service_id = s.service_id "
				+ "WHERE c.customer_id = ? " + "ORDER BY ci.item_id DESC";
		...
		try (Connection conn = DBConnection.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					...
					items.add(new CartItem(rs.getInt("item_id"), rs.getInt("service_id"), rs.getString("name"),
							rs.getDouble("price"), rs.getInt("quantity"), rs.getTimestamp("start_time"),
							rs.getTimestamp("end_time"), caregiverId, rs.getString("special_request")));
				}
			}
