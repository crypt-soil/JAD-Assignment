...
public class EmergencyContactDAO {
	// Retrieves all emergency contacts associated with a specific customer ID,
	// ordered by contact ID
	public List<EmergencyContact> getByCustomerId(int customerId) {
		...
		try {
			...
			while (rs.next()) {
				list.add(new EmergencyContact(rs.getInt("contact_id"), rs.getInt("customer_id"),
						rs.getString("contact_name"), rs.getString("relationship"), rs.getString("phone"),
						rs.getString("email")));
			}
	...
	// Inserts a new emergency contact record linked to the specified customer
	...
	// Modifies an existing emergency contact with verification that the contact
	// belongs to the customer
	public void updateContact(int contactId, int customerId, String name, String relationship, String phone,
			String email) {
		try {
			Connection conn = DBConnection.getConnection();
	...
	// Removes an emergency contact record after verifying ownership by the customer
