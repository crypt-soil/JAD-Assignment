...
String loadSql = "SELECT ci.quantity, ci.start_time, ci.end_time, "
...
if (startParam != null && endParam != null && !startParam.isEmpty() && !endParam.isEmpty()) {
	...
	if (newEnd.before(newStart)) {
		...
		String overlapSql = "SELECT 1 " + "FROM booking_details bd "
		+ "JOIN bookings b ON bd.booking_id = b.booking_id " + "WHERE b.customer_id = ? "
		+ "  AND bd.service_id = ? " + "  AND b.status IN (1,2,3) " + // pending/confirmed/completed
	...
	// ---- UPDATE cart_items ----
	String updateSql = "UPDATE cart_items " + "SET quantity = ?, start_time = ?, end_time = ? "
			+ "WHERE item_id = ?";
	...
	<div class="wrapper">
		<div class="form-card">
			...
			<form method="post">
				...
				<!-- datetime -->
				<div class="mb-3">
					<label for="startDateTime" class="label-text">Start Date &
				...
				<div class="mb-4">
					...
					<small class="text-muted"> Final amount will be confirmed
						at checkout based on your selected quantity. </small>
				</div>
